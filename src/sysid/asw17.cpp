...
/**
 * @brief Actuator control using offboard mode for system ID of the ASW-17
 * @file asw17.cpp
...
void OffboardControl::publish_actuators()
{
	...
	msg_motors.control[0] = 0.5*(dt + 1.0); // motor
	...
	msg_servos.control[1] += input[0];  // right aileron
	msg_servos.control[2] += -input[1]; // elevator
	msg_servos.control[3] += input[2];  // rudder
